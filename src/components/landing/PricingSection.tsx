...
const PricingSection = () => {
  return (
    <section id="planos" className="py-24 bg-surface-dark relative">
          ...
          {plans.map((plan, i) => (
            <motion.div
              ...
              className={`relative rounded-2xl p-8 border transition-all duration-300 ${
                plan.highlighted
                  ? "bg-surface-dark-card border-primary/40 shadow-premium scale-105"
                  : "bg-surface-dark-card border-border/10 hover:border-primary/20"
              }`}
            ...
            >
              ...
              <h3 className="font-display font-bold text-xl text-on-dark mb-2">{plan.name}</h3>
              <p className="text-on-dark-muted text-sm font-body mb-6">{plan.description}</p>

              <div className="mb-8">
                <span className="font-display font-black text-4xl text-on-dark">{plan.price}</span>
                <span className="text-on-dark-muted text-sm">{plan.period}</span>
              </div>
              ...
              <ul className="space-y-3 mb-8">
                {plan.features.map((f, j) => (
                  <li key={j} className="flex items-start gap-3 text-sm font-body">
                    ...
                    <span className="text-on-dark-muted">{f}</span>
                  </li>
                ))}
              </ul>
