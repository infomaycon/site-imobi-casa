...
const SettingsPage = () => {
  ...
  const [form, setForm] = useState({ domain: "", site_name: "", whatsapp: "" });

  useEffect(() => {
    if (!user) return;
    supabase.from("site_settings").select("domain, site_name, whatsapp").single().then(({ data }) => {
      if (data) setForm({ domain: data.domain || "", site_name: data.site_name || "", whatsapp: data.whatsapp || "" });
    });
  }, [user]);
